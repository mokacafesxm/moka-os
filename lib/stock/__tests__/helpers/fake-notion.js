...
// In-memory fake of the app/api/_notion.js CRUD/query surface used by lib/stock.
// Never touches the network — mirrors the real Notion property shapes
// closely enough (write shorthand -> `{type, ...}` on read) for the ledger
// and quantity readers in lib/stock/apply-addition.js to work unmodified.
...
function normalizeProperty(raw) {
  ...
  if ('rich_text' in raw) {
    return {
      type: 'rich_text',
      rich_text: raw.rich_text.map((t) => ({ plain_text: t.text?.content ?? '', text: t.text })),
    };
  }
  if ('title' in raw) {
    return {
      type: 'title',
      title: raw.title.map((t) => ({ plain_text: t.text?.content ?? '', text: t.text })),
    };
  }
...
function normalizeProperties(rawProperties) {
  ...
  for (const [key, value] of Object.entries(rawProperties || {})) {
    out[key] = normalizeProperty(value);
  }
  return out;
}
...
function createFakeNotion(seedPages = {}) {
  ...
  let counter = 0;
  const calls = { getPage: 0, updatePage: 0, createPage: 0, resolveName: 0, queryDatabase: 0 };
  ...
  return { getPage, updatePage, createPage, resolveName, queryDatabase, _pages: pages, _calls: calls };
}
