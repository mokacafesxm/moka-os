...
describe('classifyDocument — unambiguous rule-based recognition', () => {
  ...
  it('recognizes a sales journal (title + UTILISATEURS section) as pos_export with enough confidence to reach the parser', () => {
    const text = 'Journal de ventes UTILISATEURS GUILLAUME MOKA GAETANO R ticket moyen 4.20';
    const { scores, posSourceMatches } = applyDeterministicRules(text);

    expect(posSourceMatches.pos_export).toBe('addictill_export');
    expect(scores.pos_export).toBeGreaterThanOrEqual(AUTO_THRESHOLD);
  });
});
