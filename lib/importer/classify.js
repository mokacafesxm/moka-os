...
const RULES = {
  ...
  pos_export: [
    ...
    // "Journal de ventes" — a third AddicTill export (cumulative, long date
    // range, per-employee "UTILISATEURS" breakdown), distinct from daily
    // summary/product ranking (see parsers/pos-addictill.js's
    // detectAddicTillReportType). Weighted high enough that the title alone
    // (0.6) or the section alone (0.35) plus one shared KPI keyword above
    // reaches AUTO_THRESHOLD — this signal only gets a document as far as
    // "recognized pos_export", the parser itself still refuses to invent an
    // extraction for it (no real export available to verify column layout).
    { signal: 'journal_de_ventes_keyword', weight: 0.6, pattern: /journal\s+de\s+ventes/i, posSource: 'addictill_export' },
    { signal: 'utilisateurs_keyword', weight: 0.35, pattern: /\butilisateurs\b/i, posSource: 'addictill_export' },
  ],
  monthly_performance: [
