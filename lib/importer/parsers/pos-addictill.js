 ...
 * names/content — never guessed silently: returns null if no signature
 * matches. `sales_journal` ("Journal de ventes") is recognized by signature
 * only — see parsePosAddictill for why it is not parsed yet.
 * @param {import('../schemas').ExtractionResult} extraction
 * @returns {'daily_summary'|'product_ranking'|'sales_journal'|null}
 */
function detectAddicTillReportType(extraction) {
  ...
  const firstCellText = headerText(firstCell).toLowerCase();
  if (firstCellText.includes('synthese quotidienne')) return 'daily_summary';

  // "Journal de ventes" — cumul sur une période large (souvent plusieurs
  // mois) avec une section "UTILISATEURS" (répartition par employé), signe
  // distinctif absent de la synthèse quotidienne. Reconnu par signature
  // uniquement : jamais confirmé contre un vrai export (pas de fichier
  // disponible), donc jamais parsé — voir parsePosAddictill.
  const hasJournalTitle = firstCellText.includes('journal de ventes');
  const hasUtilisateursSection = extraction.sheets.some((s) =>
    s.rows.some((row) => row.some((cell) => headerText(cell).toLowerCase().includes('utilisateurs')))
  );
  if (hasJournalTitle || (hasUtilisateursSection && !firstCellText.includes('synthese'))) return 'sales_journal';
 ...
 * @param {import('../schemas').ExtractionResult} extraction
 * @param {{periodStart?: string, periodEnd?: string}} [options]
 * @returns {{reportType: 'daily_summary'|'product_ranking'|'sales_journal'|null, statement: Object|null, validation: import('../schemas').ValidationResult}}
 */
function parsePosAddictill(extraction, options = {}) {
  ...
  if (reportType === 'sales_journal') {
    // Reconnu par signature (titre "Journal de ventes" et/ou section
    // "UTILISATEURS") mais jamais parsé : contrairement à daily_summary et
    // product_ranking (chacun "confirmé contre un vrai export" avant d'être
    // encodé, voir l'en-tête de ce fichier), aucun fichier réel n'a permis
    // de vérifier la disposition exacte des colonnes de ce rapport. Plutôt
    // que de deviner une structure de colonnes pour un rapport financier,
    // ce type reste explicitement non implémenté — voir docs/ARCHITECTURE.md.
    return {
      reportType,
      statement: null,
      validation: ValidationResultSchema.parse({
        valid: false,
        errors: [
          'ADDICTILL_REPORT_NOT_YET_SUPPORTED: "Journal de ventes" reconnu (titre et/ou section UTILISATEURS) ' +
            'mais son extraction n\'est pas encore implémentée — la disposition exacte des colonnes n\'a jamais ' +
            'été vérifiée contre un fichier réel. Fournir un export réel pour finaliser ce parseur.',
        ],
        warnings: [],
      }),
    };
  ...
  return {
    ...
    validation: ValidationResultSchema.parse({
      valid: false,
      errors: [
        'UNSUPPORTED_ADDICTILL_REPORT: ni "Synthèse quotidienne", "Palmarès produits" ni "Journal de ventes" reconnu dans ce fichier',
      ],
      warnings: [],
    }),
  };
