...
// pdf-parse (via its pdfjs-dist dependency) references the global
// `DOMMatrix` at its OWN module-evaluation time — before any PDF is parsed,
// on the text-only extraction path this file actually uses (.getText()/
// .getTable(), never canvas rendering — see next.config.mjs's
// serverExternalPackages comment). `serverExternalPackages` correctly stops
// Turbopack from bundling pdf-parse, but that alone doesn't help here:
// `DOMMatrix` genuinely doesn't exist as a Node global in Vercel's
// production serverless runtime (confirmed 2026-07-31 — every route that
// imports this file crashed with "ReferenceError: DOMMatrix is not
// defined" at `require('pdf-parse')`, for EVERY request regardless of file
// type, including requests that never touch the PDF path at all, since the
// crash happens at import time, not call time). A minimal stub is safe
// specifically because the real canvas-rendering path that would need a
// genuine DOMMatrix is never invoked.
