...
function getFileUrl(page, propName) {
  const prop = page.properties?.[propName];
  const file = prop?.type === 'files' ? prop.files?.[0] : null;
  return file ? file.file?.url || file.external?.url || '' : '';
}
function getNumber(page, propName) {
  const prop = page.properties?.[propName];
  return prop?.type === 'number' && prop.number !== null && prop.number !== undefined ? prop.number : 0;
}
...
function mapPage(page) {
  return {
    ...
    photoUrl: getFileUrl(page, 'photo'),
    targetTimeMin: getNumber(page, 'target_time_min'),
  };
}
