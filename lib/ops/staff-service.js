...
function mapPage(page) {
  ...
  return {
    ...
    // "Poste" (Sprint 12) / "Access" (Sprint 13) — real select/multi_select
    // fields, distinct from the free-text "Rôle" above. See app/api/staff/route.js.
    poste: p['Poste']?.select?.name || '',
    access: (p['Access']?.multi_select || []).map((o) => o.name),
...
function buildStaffProperties(data) {
  ...
  if ('poste' in (data || {})) props['Poste'] = data.poste ? { select: { name: data.poste } } : { select: null };
  if ('access' in (data || {})) props['Access'] = { multi_select: (data.access || []).map((name) => ({ name })) };
  return props;
}
