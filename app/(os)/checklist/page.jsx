...
// Écran de complétion des checklists opérationnelles (Bar Manager Operating
// System v1.0) — une tâche à la fois, même pattern visuel que WorkflowRunner
// (barre de progression, une carte, Retour/Valider), mais piloté par les
// données Notion (Bar_Checklist_Templates/Instances) plutôt que des étapes
// codées en dur. La décision "Non conforme" (ex. température hors plage) et
// la déclaration d'incident qui en découle sont calculées côté serveur —
...
export default function ChecklistPage() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/checklist-status?staffId=${staffId}`)
      ...
      .catch((err) => {
        console.error("[ChecklistPage] fetch failed", err);
        setItems([]);
      });
  }, [staffId]);
