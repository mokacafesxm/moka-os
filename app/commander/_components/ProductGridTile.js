...
// Grid-friendly sibling of ProductCard.js (which stays fixed-width for the
// horizontal-scroll Favoris list) — same visual language, sized to fill a
// CSS grid cell instead.
export default function ProductGridTile({ product, onSelect, favorited, onToggleFavorite }) {
  ...
  return (
    <div
      ...
      className="cursor-pointer focus:outline-none focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:ring-[#587F25] rounded-3xl"
    >
      <div className="relative w-full aspect-square rounded-3xl overflow-hidden shadow-md bg-white active:scale-[0.97] transition-transform">
        {product.photo ? (
          <Image
            src={product.photo}
            alt={product.nom}
